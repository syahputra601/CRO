...
<h2 style="text-align: center;">Bulan April Tahun <?= @$get_year_filter ?></h2>
<center>
<!-- <?php
...
//   echo "<br>";
// }
function generateSalesService($data=''){//not implement
...
foreach($DataCompanyAreaCabang as $row_company){
  ...
  foreach($DataCabang as $row_cabang){
    ...
    //   echo "<br>";
    // }
    $join_month_year = $get_year_filter.'-04';//for april
    // print_r($join_month_year);
    // echo "<br>";
    // $DataReportBulanan = $this->db->query("SELECT COUNT(c.kode_kantor) AS jml_data_area, COUNT(e.type) AS jml_data, a.nama_cust, c.kode_kantor, c.kota, e.no_type, e.type, e.quest, COUNT(IF(d.nilai = '0', 0, NULL)) 'Kosong', COUNT(IF(d.nilai = '1', 0, NULL)) 'Pertama', COUNT(IF(d.nilai = '2', 0, NULL)) 'Dua', COUNT(IF(d.nilai = '3', 0, NULL)) 'Tiga', COUNT(IF(d.nilai = '4', 0, NULL)) 'Empat', COUNT(IF(d.nilai = '5', 0, NULL)) 'Lima', COUNT(IF(d.nilai = '6', 0, NULL)) 'Enam', COUNT(IF(d.nilai = '7', 0, NULL)) 'Tujuh', COUNT(IF(d.nilai = '8', 0, NULL)) 'Delapan', COUNT(IF(d.nilai = '9', 0, NULL)) 'Sembilan', COUNT(IF(d.nilai = '10', 0, NULL)) 'Sepuluh', (SUM(0)+SUM(1)) AS Total, (SUM('0')+SUM('3')) AS Total_A FROM tb_surv AS a INNER JOIN tb_hedsurv AS b ON a.id_surv = b.id_surv INNER JOIN mst_company AS c ON a.id_comp = c.id_company INNER JOIN tb_detsurv AS d ON a.id_surv = d.id_surv INNER JOIN mst_quest AS e ON d.id_quest = e.id_quest WHERE c.kode_kantor = '".$KodeKantor."' AND c.kota = '".$CabangCompany."' AND b.id_respon = '6' AND a.tgl_inv LIKE '%".$join_month_year."%' GROUP BY d.id_quest ORDER BY a.tipe_cust ASC, e.no_type ASC")->result();

    $DataReportBulanan = $this->db->query("SELECT COUNT(c.kode_kantor) AS jml_data_area, COUNT(e.type) AS jml_data, a.nama_cust, c.kode_kantor, a.id_comp, c.id_company, c.nama AS nama_cabang, c.kota, e.no_type, e.type, e.quest, e.nm_quest, COUNT(IF(d.nilai = '0', 0, NULL)) 'Kosong', COUNT(IF(d.nilai = '1', 0, NULL)) 'Pertama', COUNT(IF(d.nilai = '2', 0, NULL)) 'Dua', COUNT(IF(d.nilai = '3', 0, NULL)) 'Tiga', COUNT(IF(d.nilai = '4', 0, NULL)) 'Empat', COUNT(IF(d.nilai = '5', 0, NULL)) 'Lima', COUNT(IF(d.nilai = '6', 0, NULL)) 'Enam', COUNT(IF(d.nilai = '7', 0, NULL)) 'Tujuh', COUNT(IF(d.nilai = '8', 0, NULL)) 'Delapan', COUNT(IF(d.nilai = '9', 0, NULL)) 'Sembilan', COUNT(IF(d.nilai = '10', 0, NULL)) 'Sepuluh', (SUM(0)+SUM(1)) AS Total, (SUM('0')+SUM('3')) AS Total_A FROM tb_surv AS a INNER JOIN tb_hedsurv AS b ON a.id_surv = b.id_surv INNER JOIN mst_company AS c ON a.id_comp = c.id_company INNER JOIN tb_detsurv AS d ON a.id_surv = d.id_surv INNER JOIN mst_quest AS e ON d.id_quest = e.id_quest WHERE c.kode_kantor = '".$KodeKantor."' AND c.nama = '".$CabangNama."' AND b.id_respon = '6' AND a.tgl_inv LIKE '%".$join_month_year."%' GROUP BY d.id_quest ORDER BY a.tipe_cust ASC, e.no_type ASC")->result();
    $DataSemuaArea = $this->db->query("SELECT c.kode_kantor, a.id_comp, c.id_company, c.nama AS nama_cabang, c.kota FROM tb_surv AS a INNER JOIN tb_hedsurv AS b ON a.id_surv = b.id_surv INNER JOIN mst_company AS c ON a.id_comp = c.id_company INNER JOIN tb_detsurv AS d ON a.id_surv = d.id_surv INNER JOIN mst_quest AS e ON d.id_quest = e.id_quest WHERE c.kode_kantor = '".$KodeKantor."' AND c.nama = '".$CabangNama."' AND b.id_respon = '6' AND a.tgl_inv LIKE '%".$join_month_year."%' GROUP BY c.kode_kantor")->result();
    // echo "<br>";
    // echo $this->db->last_query($DataSemuaArea);
    ...
    $no_kd_kantor=0;
    foreach ($DataReportBulanan as $key_number_rata) {
      if($key_number_rata->kode_kantor == $KodeKantor){
        $no_kd_kantor++;
      }
      // print_r($no_kd_kantor);
      $Ary_no_kd_kantor[] = $no_kd_kantor;
      foreach ($Ary_no_kd_kantor as $row_no_area) {
        $kd_kantor_no = $row_no_area;
      }
      // print_r($no_kd_kantor);
    }
...
<!-- <table class="responsive-table" style="width: 100%;">
  ...
  <caption style="text-align: left; font-size: 17px;">Nama Cabang2 : <?= @$row_cabang->nama ?></caption> -->
  <tbody>
<?php
$no=0;
$startTotal=0;
...
// foreach ($Data_Ary_all_area as $area_all_above_datareportbulanan) {
  
// }
foreach($DataReportBulanan as $row){
$cek_emptydata_new++;
$no++;
// if($row->kode_kantor == "JKT"){
  // $no_kd_kantor2++;
// }
...
?>
  ...
  <tr>
    <td data-title="Area">
      ...
      ?>
      <?php echo "||".$row->kode_kantor."||".$KodeKantor."||".@$cek_emptydata_new ?> -->
      <?php echo $row->kode_kantor ?>
    </td>
    ...
    ?>
    <!-- <td data-title="-"><?= @$no_kd_kantor2 ?></td> -->
  </tr>
...
if(@$cek_emptydata_new == 0){
  echo "<center><p style='color:red;'>Tidak Ada Laporan Survey Semua Area Pada Bulan April.</p></center>";
//start coding bagian hitung Rata-Rata
  //start for service
...
?>
      <!-- <table border="1" style="width: 100%;">
        ...
        <br>
<?php
// print_r($Array_DataSemuaArea);
// echo "<br>";
// print_r(@$ArrayGetTotal);
// echo "<br>";
// print_r(@$Array_GetSalesNoType_1_sumsel);
// echo "<br>";
//start version old
    //start for service
...
<?php
// $tesNAN = 'NAN';
// echo $Rata_rata_ServiceNoType_1_sumsel;
// echo "<br>";
// echo is_numeric($Rata_rata_ServiceNoType_1_sumsel);
// echo "<br>";
// echo $Rata_rata_SalesNoType_4_kalsel;
// echo "<br>";
// echo is_numeric($Rata_rata_SalesNoType_4_kalsel);
// echo "<br>";
// if(!is_nan($Rata_rata_SalesNoType_4_kalsel) && $Rata_rata_SalesNoType_4_kalsel == 0){
//   echo "NOT NAN";
// }elseif(is_nan($Rata_rata_SalesNoType_4_kalsel) && $Rata_rata_SalesNoType_4_kalsel != 0){
...
<!-- <br> -->
<!-- START VERSION OLD -->
<input type="hidden" name="txt_Rata_rata_ServiceNoType_1_april" id="txt_Rata_rata_ServiceNoType_1_april" value="<?= $Fix_Rata_rata_ServiceNoType_1 ?>" readonly>
<input type="hidden" name="txt_Rata_rata_ServiceNoType_2_april" id="txt_Rata_rata_ServiceNoType_2_april" value="<?= @$Fix_Rata_rata_ServiceNoType_2 ?>" readonly>
<input type="hidden" name="txt_Rata_rata_ServiceNoType_3_april" id="txt_Rata_rata_ServiceNoType_3_april" value="<?= @$Fix_Rata_rata_ServiceNoType_3 ?>" readonly>
<input type="hidden" name="txt_Rata_rata_ServiceNoType_4_april" id="txt_Rata_rata_ServiceNoType_4_april" value="<?= @$Fix_Rata_rata_ServiceNoType_4 ?>" readonly>
<!-- <br> -->
<input type="hidden" name="txt_Rata_rata_SalesNoType_1_april" id="txt_Rata_rata_SalesNoType_1_april" value="<?= @$Fix_Rata_rata_SalesNoType_1 ?>" readonly>
<input type="hidden" name="txt_Rata_rata_SalesNoType_2_april" id="txt_Rata_rata_SalesNoType_2_april" value="<?= @$Fix_Rata_rata_SalesNoType_2 ?>" readonly>
<input type="hidden" name="txt_Rata_rata_SalesNoType_3_april" id="txt_Rata_rata_SalesNoType_3_april" value="<?= @$Fix_Rata_rata_SalesNoType_3 ?>" readonly>
<input type="hidden" name="txt_Rata_rata_SalesNoType_4_april" id="txt_Rata_rata_SalesNoType_4_april" value="<?= @$Fix_Rata_rata_SalesNoType_4 ?>" readonly>
<!-- END VERSION OLD -->
<!-- START BAGIAN TEXT INPUT -->
  <!-- START FOR JAKARTA -->
<input type="hidden" name="txt_Rata_rata_ServiceNoType_1_april_jkt" id="txt_Rata_rata_ServiceNoType_1_april_jkt" value="<?= $Fix_Rata_rata_ServiceNoType_1_jkt ?>" readonly>
<input type="hidden" name="txt_Rata_rata_ServiceNoType_2_april_jkt" id="txt_Rata_rata_ServiceNoType_2_april_jkt" value="<?= @$Fix_Rata_rata_ServiceNoType_2_jkt ?>" readonly>
<input type="hidden" name="txt_Rata_rata_ServiceNoType_3_april_jkt" id="txt_Rata_rata_ServiceNoType_3_april_jkt" value="<?= @$Fix_Rata_rata_ServiceNoType_3_jkt ?>" readonly>
<input type="hidden" name="txt_Rata_rata_ServiceNoType_4_april_jkt" id="txt_Rata_rata_ServiceNoType_4_april_jkt" value="<?= @$Fix_Rata_rata_ServiceNoType_4_jkt ?>" readonly>
<!-- <br> -->
<input type="hidden" name="txt_Rata_rata_SalesNoType_1_april_jkt" id="txt_Rata_rata_SalesNoType_1_april_jkt" value="<?= @$Fix_Rata_rata_SalesNoType_1_jkt ?>" readonly>
<input type="hidden" name="txt_Rata_rata_SalesNoType_2_april_jkt" id="txt_Rata_rata_SalesNoType_2_april_jkt" value="<?= @$Fix_Rata_rata_SalesNoType_2_jkt ?>" readonly>
<input type="hidden" name="txt_Rata_rata_SalesNoType_3_april_jkt" id="txt_Rata_rata_SalesNoType_3_april_jkt" value="<?= @$Fix_Rata_rata_SalesNoType_3_jkt ?>" readonly>
<input type="hidden" name="txt_Rata_rata_SalesNoType_4_april_jkt" id="txt_Rata_rata_SalesNoType_4_april_jkt" value="<?= @$Fix_Rata_rata_SalesNoType_4_jkt ?>" readonly>
  <!-- END FOR JAKARTA -->
  <!-- START FOR KALTIM -->
<input type="hidden" name="txt_Rata_rata_ServiceNoType_1_april_kaltim" id="txt_Rata_rata_ServiceNoType_1_april_kaltim" value="<?= $Fix_Rata_rata_ServiceNoType_1_kaltim ?>" readonly>
<input type="hidden" name="txt_Rata_rata_ServiceNoType_2_april_kaltim" id="txt_Rata_rata_ServiceNoType_2_april_kaltim" value="<?= @$Fix_Rata_rata_ServiceNoType_2_kaltim ?>" readonly>
<input type="hidden" name="txt_Rata_rata_ServiceNoType_3_april_kaltim" id="txt_Rata_rata_ServiceNoType_3_april_kaltim" value="<?= @$Fix_Rata_rata_ServiceNoType_3_kaltim ?>" readonly>
<input type="hidden" name="txt_Rata_rata_ServiceNoType_4_april_kaltim" id="txt_Rata_rata_ServiceNoType_4_april_kaltim" value="<?= @$Fix_Rata_rata_ServiceNoType_4_kaltim ?>" readonly>
<!-- <br> -->
<input type="hidden" name="txt_Rata_rata_SalesNoType_1_april_kaltim" id="txt_Rata_rata_SalesNoType_1_april_kaltim" value="<?= @$Fix_Rata_rata_SalesNoType_1_kaltim ?>" readonly>
<input type="hidden" name="txt_Rata_rata_SalesNoType_2_april_kaltim" id="txt_Rata_rata_SalesNoType_2_april_kaltim" value="<?= @$Fix_Rata_rata_SalesNoType_2_kaltim ?>" readonly>
<input type="hidden" name="txt_Rata_rata_SalesNoType_3_april_kaltim" id="txt_Rata_rata_SalesNoType_3_april_kaltim" value="<?= @$Fix_Rata_rata_SalesNoType_3_kaltim ?>" readonly>
<input type="hidden" name="txt_Rata_rata_SalesNoType_4_april_kaltim" id="txt_Rata_rata_SalesNoType_4_april_kaltim" value="<?= @$Fix_Rata_rata_SalesNoType_4_kaltim ?>" readonly>
  <!-- END FOR KALTIM -->
  <!-- START FOR KALBAR -->
<input type="hidden" name="txt_Rata_rata_ServiceNoType_1_april_kalbar" id="txt_Rata_rata_ServiceNoType_1_april_kalbar" value="<?= $Fix_Rata_rata_ServiceNoType_1_kalbar ?>" readonly>
<input type="hidden" name="txt_Rata_rata_ServiceNoType_2_april_kalbar" id="txt_Rata_rata_ServiceNoType_2_april_kalbar" value="<?= @$Fix_Rata_rata_ServiceNoType_2_kalbar ?>" readonly>
<input type="hidden" name="txt_Rata_rata_ServiceNoType_3_april_kalbar" id="txt_Rata_rata_ServiceNoType_3_april_kalbar" value="<?= @$Fix_Rata_rata_ServiceNoType_3_kalbar ?>" readonly>
<input type="hidden" name="txt_Rata_rata_ServiceNoType_4_april_kalbar" id="txt_Rata_rata_ServiceNoType_4_april_kalbar" value="<?= @$Fix_Rata_rata_ServiceNoType_4_kalbar ?>" readonly>
<!-- <br> -->
<input type="hidden" name="txt_Rata_rata_SalesNoType_1_april_kalbar" id="txt_Rata_rata_SalesNoType_1_april_kalbar" value="<?= @$Fix_Rata_rata_SalesNoType_1_kalbar ?>" readonly>
<input type="hidden" name="txt_Rata_rata_SalesNoType_2_april_kalbar" id="txt_Rata_rata_SalesNoType_2_april_kalbar" value="<?= @$Fix_Rata_rata_SalesNoType_2_kalbar ?>" readonly>
<input type="hidden" name="txt_Rata_rata_SalesNoType_3_april_kalbar" id="txt_Rata_rata_SalesNoType_3_april_kalbar" value="<?= @$Fix_Rata_rata_SalesNoType_3_kalbar ?>" readonly>
<input type="hidden" name="txt_Rata_rata_SalesNoType_4_april_kalbar" id="txt_Rata_rata_SalesNoType_4_april_kalbar" value="<?= @$Fix_Rata_rata_SalesNoType_4_kalbar ?>" readonly>
  <!-- END FOR KALBAR -->
  <!-- START FOR KALSEL -->
<input type="hidden" name="txt_Rata_rata_ServiceNoType_1_april_kalsel" id="txt_Rata_rata_ServiceNoType_1_april_kalsel" value="<?= $Fix_Rata_rata_ServiceNoType_1_kalsel ?>" readonly>
<input type="hidden" name="txt_Rata_rata_ServiceNoType_2_april_kalsel" id="txt_Rata_rata_ServiceNoType_2_april_kalsel" value="<?= @$Fix_Rata_rata_ServiceNoType_2_kalsel ?>" readonly>
<input type="hidden" name="txt_Rata_rata_ServiceNoType_3_april_kalsel" id="txt_Rata_rata_ServiceNoType_3_april_kalsel" value="<?= @$Fix_Rata_rata_ServiceNoType_3_kalsel ?>" readonly>
<input type="hidden" name="txt_Rata_rata_ServiceNoType_4_april_kalsel" id="txt_Rata_rata_ServiceNoType_4_april_kalsel" value="<?= @$Fix_Rata_rata_ServiceNoType_4_kalsel ?>" readonly>
<!-- <br> -->
<input type="hidden" name="txt_Rata_rata_SalesNoType_1_april_kalsel" id="txt_Rata_rata_SalesNoType_1_april_kalsel" value="<?= @$Fix_Rata_rata_SalesNoType_1_kalsel ?>" readonly>
<input type="hidden" name="txt_Rata_rata_SalesNoType_2_april_kalsel" id="txt_Rata_rata_SalesNoType_2_april_kalsel" value="<?= @$Fix_Rata_rata_SalesNoType_2_kalsel ?>" readonly>
<input type="hidden" name="txt_Rata_rata_SalesNoType_3_april_kalsel" id="txt_Rata_rata_SalesNoType_3_april_kalsel" value="<?= @$Fix_Rata_rata_SalesNoType_3_kalsel ?>" readonly>
<input type="hidden" name="txt_Rata_rata_SalesNoType_4_april_kalsel" id="txt_Rata_rata_SalesNoType_4_april_kalsel" value="<?= @$Fix_Rata_rata_SalesNoType_4_kalsel ?>" readonly>
  <!-- END FOR KALSEL -->
  <!-- START FOR SUMSEL -->
<input type="hidden" name="txt_Rata_rata_ServiceNoType_1_april_sumsel" id="txt_Rata_rata_ServiceNoType_1_april_sumsel" value="<?= $Fix_Rata_rata_ServiceNoType_1_sumsel ?>" readonly>
<input type="hidden" name="txt_Rata_rata_ServiceNoType_2_april_sumsel" id="txt_Rata_rata_ServiceNoType_2_april_sumsel" value="<?= @$Fix_Rata_rata_ServiceNoType_2_sumsel ?>" readonly>
<input type="hidden" name="txt_Rata_rata_ServiceNoType_3_april_sumsel" id="txt_Rata_rata_ServiceNoType_3_april_sumsel" value="<?= @$Fix_Rata_rata_ServiceNoType_3_sumsel ?>" readonly>
<input type="hidden" name="txt_Rata_rata_ServiceNoType_4_april_sumsel" id="txt_Rata_rata_ServiceNoType_4_april_sumsel" value="<?= @$Fix_Rata_rata_ServiceNoType_4_sumsel ?>" readonly>
<!-- <br> -->
<input type="hidden" name="txt_Rata_rata_SalesNoType_1_april_sumsel" id="txt_Rata_rata_SalesNoType_1_april_sumsel" value="<?= @$Fix_Rata_rata_SalesNoType_1_sumsel ?>" readonly>
<input type="hidden" name="txt_Rata_rata_SalesNoType_2_april_sumsel" id="txt_Rata_rata_SalesNoType_2_april_sumsel" value="<?= @$Fix_Rata_rata_SalesNoType_2_sumsel ?>" readonly>
<input type="hidden" name="txt_Rata_rata_SalesNoType_3_april_sumsel" id="txt_Rata_rata_SalesNoType_3_april_sumsel" value="<?= @$Fix_Rata_rata_SalesNoType_3_sumsel ?>" readonly>
<input type="hidden" name="txt_Rata_rata_SalesNoType_4_april_sumsel" id="txt_Rata_rata_SalesNoType_4_april_sumsel" value="<?= @$Fix_Rata_rata_SalesNoType_4_sumsel ?>" readonly>
  <!-- END FOR SUMSEL -->
...
<?php
  ...
  if(@$row_mst_quest->type == 'SALES'){
    ...
    //START FOR SALES
    echo "<td>";
    if($row_mst_quest->no_type == 1 && $row_ary_all_area == "JKT"){
